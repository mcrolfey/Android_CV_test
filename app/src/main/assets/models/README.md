...
match the reference batch pipeline exactly (see `InferenceManager`/`AsbestosClassifier` for where
each threshold is set):

- `roi_detector.onnx` — from `cement_roi_model.pt`. YOLO ROI detector. conf 0.25, iou 0.5.
  Architecture A takes only the top-1 box; Architecture B (standalone baseline) is uncapped.
...
- `pa_detector.onnx` — from `pa_detector.pt`. YOLO "potential asbestos" detector, run on the ROI
  crop (not the full frame). conf 0.10, iou 0.5, up to 100 boxes.
...
- `resnet_binary.onnx` — from `resnet_A_NA_best.pt`. ResNet-18, 2-class (A=0, NA-OF=1), input
  1x3x224x224, ImageNet mean/std. If P(A) < 0.4 the cascade stops here and reports "NA-OF".
- `resnet_subtype.onnx` — from `resnet_A_3class_best.pt`. ResNet-18, 3-class
  (A-AM=0, A-C=1, A-CRO=2), same input/normalization. Only runs when P(A) >= 0.4; falls back to
  the generic "A" label if its top confidence is below 0.25.
